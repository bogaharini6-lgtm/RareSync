  ...
  try {
    ...
    if (req.user.role === 'doctor') {
      ...
      if (hospitals.length && doctors.length) {
        sendEmail({ to: hospitals[0].email, subject: 'New patient added - ' + name, html: templates.patientAdded({ hospitalName: hospitals[0].name, doctorName: doctors[0].name, patientName: name, gender, bloodGroup: blood_group, addedAt: new Date() }) });
      }
    }
  ...
  try {
    if (req.user.role === 'hospital') {
      const [rows] = await db.execute(
        'SELECT p.*, h.name AS hospital_name, GROUP_CONCAT(DISTINCT rd.name SEPARATOR ", ") AS disease_names FROM patients p JOIN hospitals h ON p.hospital_id = h.id LEFT JOIN patient_diseases pd ON pd.patient_id = p.id LEFT JOIN rare_diseases rd ON rd.id = pd.disease_id WHERE p.hospital_id = ? AND (p.name LIKE ? OR p.contact LIKE ?) AND p.deleted_at IS NULL GROUP BY p.id ORDER BY p.created_at DESC LIMIT ' + limit + ' OFFSET ' + offset,
    ...
    const [rows] = await db.execute(
      'SELECT p.id, p.name, p.dob, p.gender, p.hospital_id, p.created_at, p.created_by_doctor, h.name AS hospital_name, GROUP_CONCAT(DISTINCT rd.name SEPARATOR ", ") AS disease_names FROM patients p JOIN hospitals h ON p.hospital_id = h.id LEFT JOIN patient_diseases pd ON pd.patient_id = p.id LEFT JOIN rare_diseases rd ON rd.id = pd.disease_id WHERE (p.name LIKE ? OR h.name LIKE ?) AND p.deleted_at IS NULL GROUP BY p.id ORDER BY p.created_at DESC LIMIT ' + limit + ' OFFSET ' + offset,
      ['%' + search + '%', '%' + search + '%']
    );
...
exports.getPatientById = async (req, res) => {
  ...
  const [hospitals] = await db.execute('SELECT name FROM hospitals WHERE id = ?', [patient.hospital_id]);
  ...
  if (accessLevel === 'limited') {
    const [diseases] = await db.execute('SELECT rd.name FROM patient_diseases pd JOIN rare_diseases rd ON pd.disease_id = rd.id WHERE pd.patient_id = ?', [patient.id]);
    return res.json({ id: patient.id, name: patient.name, age, gender: patient.gender, hospital_id: patient.hospital_id, hospital_name, disease_names: diseases.map((d) => d.name).join(', '), access_level: 'limited' });
  }
  res.json({ ...patient, age, hospital_name, access_level: 'full' });
};

exports.updatePatient = async (req, res) => {
  if (req.accessLevel !== 'full') return res.status(403).json({ message: 'You do not have full access to edit this patient.' });
  const { name, dob, gender, contact, address, blood_group, emergency_contact } = req.body;
  if (!name) return res.status(400).json({ message: 'Patient name is required.' });
  try {
...
exports.deletePatient = async (req, res) => {
  try {
    ...
    await db.execute('UPDATE patients SET deleted_at = NOW() WHERE id = ?', [req.params.id]);
